...
const todoTableContainer = document.querySelector('.todo-table-container');
...
const refreshTodos = async (filter, order) => {
    await todoTableManager.clear();
    const todos = await todoService.getAll(filter, order);
    await todoTableManager.addAll(todos, async (todo) => {
        TodoModalDialog.openEdit(todo, async (todo) => {
            try {
...
const newTodoButton = document.querySelector('.btn-new-todo');
newTodoButton.addEventListener('click', () => {
    TodoModalDialog.open(async (todo) => {
        try {
